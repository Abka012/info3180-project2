...
def calculate_match_score(current_user_profile, other_profile):
    ...
    # Location proximity (0-25 points)
    if (current_user_profile.latitude and current_user_profile.longitude and 
        other_profile.latitude and other_profile.longitude):
        distance = haversine_distance(
            current_user_profile.latitude, current_user_profile.longitude,
            other_profile.latitude, other_profile.longitude
        )
        radius = current_user_profile.location_radius or 50
        if distance <= radius:
            score += 25
        ...
        # No GPS - check location string match
        if (current_user_profile.location and other_profile.location and
            current_user_profile.location.lower() == other_profile.location.lower()):
            score += 25
            details['location'] = 'same_city'
    
    # Age compatibility (0-20 points)
    other_age = other_profile.age
    if (other_age >= current_user_profile.preferred_age_min and 
        other_age <= current_user_profile.preferred_age_max):
        score += 20
    ...
    # Relationship goal (0-20 points)
    if (current_user_profile.relationship_goal and 
        current_user_profile.relationship_goal == other_profile.relationship_goal):
        score += 20
...
def get_potential_matches():
    ...
    interacted_ids = db.session.query(Like.from_user_id).filter(
...
def like_user(to_user_id):
    ...
    if is_mutual:
        ...
        if socket_emit:
            socket_emit(to_user_id, 'new_match', {
                'user_id': user.id,
                'profile': from_profile.to_dict()
            })
            socket_emit(user.id, 'new_match', {
                'user_id': to_user_id,
                'profile': to_profile.to_dict()
            })
        ...
        if socket_emit:
            socket_emit(to_user_id, 'new_like', {
                'user_id': user.id,
                'profile': from_profile.to_dict()
            })
